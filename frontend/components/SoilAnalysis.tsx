    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setSoilData((prev) => ({
                ph: Math.max(
                    5.0,
                    Math.min(8.5, prev.ph + (Math.random() - 0.5) * 0.2)
                ),
                moisture: Math.max(
                    0,
                    Math.min(100, prev.moisture + (Math.random() - 0.5) * 5)
                ),
                nitrogen: Math.max(
                    0,
                    Math.min(100, prev.nitrogen + (Math.random() - 0.5) * 3)
                ),
                phosphorus: Math.max(
                    0,
                    Math.min(100, prev.phosphorus + (Math.random() - 0.5) * 3)
                ),
                potassium: Math.max(
                    0,
                    Math.min(100, prev.potassium + (Math.random() - 0.5) * 3)
                ),
                temperature: Math.max(
                    15,
                    Math.min(35, prev.temperature + (Math.random() - 0.5) * 1)
                ),
            }));
    ...
    const getSoilTips = () => {
        ...
        if (soilData.ph < 6.0) {
            tips.push({
                ...
                title: "pH Too Low",
                description:
                    "Add lime to increase soil pH and improve nutrient availability.",
                icon: AlertTriangle,
            });
        ...
        if (soilData.moisture < 40) {
            tips.push({
                ...
                title: "Low Moisture",
                description:
                    "Increase irrigation frequency or add mulch to retain moisture.",
                icon: Droplets,
            });
        }
        ...
        if (soilData.nitrogen < 60) {
            tips.push({
                ...
                description:
                    "Apply nitrogen-rich fertilizer or compost to boost plant growth.",
                icon: Leaf,
            });
        }
        ...
        if (tips.length === 0) {
            tips.push({
                ...
                description:
                    "Your soil conditions are optimal for most crops. Continue current practices.",
                icon: CheckCircle,
            });
        }
